...
// Handle text commands (/start with caption)
bot.onText(/\/start (.+)/, async (msg, match) => {
    const chatId = msg.chat.id;
    const caption = match[1];

    // Look for the exact caption in mediaStorage
    const foundMedia = mediaStorage[caption];

    if (foundMedia) {
        const { file_id, caption: foundCaption, uploadDate, uploadTime, type, size } = foundMedia;

        // Generate link URL with exact caption for media retrieval
        const linkCaption = encodeURIComponent(caption); // Encode the caption for the URL

        // Send the media to the user based on type
        if (type === 'photo') {
            await bot.sendPhoto(chatId, file_id, {
                caption: `Image with Caption: \`${foundCaption}\`\n\nDetails:\n- Date Uploaded: ${uploadDate}\n- Time Uploaded: ${uploadTime}\n- Size: ${size}\n\n[Click here to view](https://t.me/${botUsername}?start=${linkCaption})`,
                parse_mode: 'Markdown'
            });
        } else if (type === 'video') {
...
// Handle media uploads (photo, video, document)
...
async function handleMediaUpload(msg) {
    const allowedChatId = 7105003930; // Chat ID to allow uploads from

    const chatId = msg.chat.id;
    if (chatId !== allowedChatId) {
        await bot.sendMessage(chatId, 'Sorry, media uploads are only allowed from specific chats.');
        return;
    }
